...
/**
 * Both halves of a board, straight from Postgres. This is what the REFRESH
 * button on the board reaches: a customer who taps it has just been stamped and
 * is asking the question the cache below is not allowed to answer.
 */
export async function fetchBoard(memberId: string): Promise<Board> {
...
/**
 * The ordinary read, cached briefly rather than tagged: a leaderboard changes
 * whenever anyone in the shop is stamped, so tagging it would mean every stamp
 * busting every customer's cache. Half a minute stale is invisible on a ranking
 * and saves the database from a full aggregate on every page view.
 */
